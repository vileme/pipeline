...
def conv1x1(in_, out):
    return nn.Conv2d(in_, out, 1)


class UnitNormalization(nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, x):
        x = torch.div(x, torch.sqrt(x.pow(2).sum()))
        return x
...
class UNet16(nn.Module):
    def __init__(self, num_classes=1, num_filters=32, pretrained=False):
        ...
        self.final = nn.Conv2d(num_filters, 32, kernel_size=1)
        self.projection_head = ProjectionHead(32, 32)
    ...
    def forward(self, x):
        ...
        x_out_mask = self.projection_head(x_out_mask)

        return x_out_mask, x_out_empty_ind1
